	  ...
	  <li class="active">菜单管理</li>
                        ...
                        <div class="pull-right">
                            <%--<div class="ui-select">
                                <select name="queryCondition" >
                                	<c:choose>
	                                	<c:when test="${queryCondition == 'name' }">
	                                		<option value="name" selected="selected" >菜单名称</option>
	                                		<option value="level" >菜单等级</option>
	                                  		<option value="pname" >上级菜单名称</option>
	                                	</c:when>
	                                  	<c:when test="${queryCondition == 'level' }">
	                                		<option value="name"  >菜单名称</option>
	                                		<option value="level" selected="selected">菜单等级</option>
	                                  		<option value="pname" >上级菜单名称</option>
	                                	</c:when>
	                                	<c:when test="${queryCondition == 'pname' }">
	                                		<option value="name"  >菜单名称</option>
	                                		<option value="level" >菜单等级</option>
	                                  		<option value="pname" selected="selected">上级菜单名称</option>
	                                	</c:when>
	                                	<c:otherwise>
	                                		<option value="name" >菜单名称</option>
	                                		<option value="level" >菜单等级</option>
                           ...
                           function search(){
                            ...
                            <a class="btn-flat new-product" href="bg/system!goAddOrUpdateMenu" >+ 新 增</a>
                        </div>
                            ...
                            <tbody>
                                ...
                                <c:forEach items="${pu.data }" var="pd" >
                                	...
                                	<tr class="${temp }" style="height: 50px;" >
	                                    ...
	                                    <td >
	                                        <i class="${pd[0].icon }" style="height: 24px;width: 24px;"></i>
	                                    </td>
	                                    <td >
	                                        <span >${pd[0].sort }</span>
	                                        <ul class="actions">
	                                            <li><i class="table-edit" onclick="location.href='bg/system!goAddOrUpdateMenu?menu.id=${pd[0].id }'"></i></li>
	                                            <%--<li><i class="table-settings"></i></li>
	                                            --%><li class="last"><i class="table-delete" onclick="del('${pd[0].id }')"></i></li>
	                                        </ul>
	                                        <script type="text/javascript">
	                                        function del(id){
	                            				$('#myModal').modal('show');
	                            				
	                            				$('#sure').attr("href","bg/system!delMenu?menu.id="+id+"&queryValue=${queryValue }");
	                                        }
	                                        </script>
	                                    </td>
	                                </tr>
			...
			<div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			  ...
			  <div class="modal-body">
			    <p id="tipContent">确定删除此条记录？如果有子菜单，则会一并删除。</p>
			  </div>
